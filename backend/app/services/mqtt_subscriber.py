"""
MQTT subscriber service — paho-mqtt v2 compatible.
"""
...
class MQTTSubscriber:
    ...
    def _on_disconnect(self, client, userdata, disconnect_flags, reason_code, properties=None):
        if reason_code != 0:
            logger.warning(f"MQTT disconnected unexpectedly ({reason_code}), will reconnect")
        else:
    ...
    def _on_message(self, client, userdata, msg):
        try:
            ...
            if topic.endswith("/status"):
                logger.debug(f"Heartbeat from {topic}")
                return
    ...
    def _process_telemetry(self, payload_str: str):
        ...
        try:
            ...
            session.commit()
            logger.info(f"Stored telemetry from {telemetry.device_id} at {ts}")
